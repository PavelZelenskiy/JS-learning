...
document.addEventListener('keydown', (event) => {
    ...
    if (key === 'Tab') {
        event.preventDefault();
        if (body.style.background !=='rgb(36, 41, 46)') {
        body.style.background = '#24292E';
        taskItems.forEach(x => x.style.color = '#ffffff');
        buttons.forEach(x => x.style.border = '1px solid #ffffff');
        } else if (body.style.background ==='rgb(36, 41, 46)') {
            body.style.background = '';
            taskItems.forEach(x => x.style.color = '');
            buttons.forEach(x => x.style.border = 'none')
        };
    };
});
